...
<div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      \ = \->query("SELECT * FROM testimonials WHERE status = 'approved' ORDER BY created_at DESC LIMIT 3");
      if (\ && \->num_rows > 0):
          while (\ = \->fetch_assoc()):
      ?>
      <div class="bg-white p-8 rounded-2xl shadow-sm border border-[#D8C7A1]/20">
        ...
        <p class="text-[#173F35]/70 italic mb-6 leading-relaxed">"<?= htmlspecialchars(\['review']) ?>"</p>
        <p class="font-serif text-[#173F35] font-medium">- <?= htmlspecialchars(\['client_name']) ?><?= \['company'] ? ', ' . htmlspecialchars(\['company']) : '' ?><?= \['country'] ? ', ' . htmlspecialchars(\['country']) : '' ?></p>
      </div>
      <?php 
          endwhile;
      else:
