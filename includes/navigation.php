...
?>
<header id="site-header" class="fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ease-in-out bg-[#173F35] shadow-md border-b border-[#D8C7A1]/20">
  <div class="max-w-7xl mx-auto px-6 lg:px-10 flex items-center justify-between h-[80px]">
    <!-- Brand Logo -->
    <a class="flex items-center gap-3 group flex-shrink-0 transition-colors duration-300 text-[#F6F3EC]" href="index.php">
      <svg width="34" height="34" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        ...
        <line x1="7" y1="11.5" x2="17" y2="11.5" stroke="#D8C7A1" stroke-width="0.5" opacity="0.45"></line>
        <line x1="4.5" y1="16.5" x2="19.5" y2="16.5" stroke="#D8C7A1" stroke-width="0.5" opacity="0.35"></line>
      </svg>
      <div>
        <div class="font-serif text-[18px] leading-none tracking-wide text-[#F6F3EC]">Green Pyramids</div>
    ...
    <nav class="hidden lg:flex items-center gap-8">
      ...
      ?>
        <a class="text-[13px] font-medium tracking-wide transition-all duration-300 relative py-1 <?= $isActive ? 'text-[#8FAE5D] font-semibold border-b-2 border-[#8FAE5D]' : 'text-[#F6F3EC]/80 hover:text-[#F6F3EC] hover:-translate-y-0.5' ?>" href="<?= $url ?>">
          <?= $label ?>
        </a>
      <?php endforeach; ?>
    </nav>
    ...
    <div class="flex items-center gap-5">
      ...
      <button id="mobile-menu-btn" class="lg:hidden w-10 h-10 flex flex-col justify-center items-center gap-[5px] rounded-full transition-colors text-[#F6F3EC] hover:bg-white/10" aria-label="Toggle menu">
...
<div id="mobile-menu-drawer" class="fixed inset-0 z-[90] bg-[#173F35] flex flex-col transition-transform duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] translate-x-full">
  <div class="flex items-center justify-between px-6 h-[80px] border-b border-[#F6F3EC]/10">
    <a class="flex items-center gap-3" href="index.php">
      ...
      <span class="font-serif text-[#F6F3EC] text-[17px]">Green Pyramids</span>
    </a>
  </div>
  <nav class="flex flex-col px-8 pt-10 gap-6 overflow-y-auto">
    <?php foreach ($navItems as $url => $label): 
      $isActive = ($currentPage === $url);
    ?>
      <a class="font-serif text-[2.2rem] leading-none transition-all duration-300 <?= $isActive ? 'text-[#8FAE5D] font-bold' : 'text-[#F6F3EC]/80 hover:text-[#F6F3EC]' ?>" href="<?= $url ?>"><?= $label ?></a>
    <?php endforeach; ?>
    <div class="mt-8">
