(() => {
    ...
    const fetchAnalytics = async (action, range) => {
        ...
        if (!response.ok || !payload?.success) {
            throw new Error(payload?.error || 'analytics_unavailable');
        }
        return payload.data;
    };
    ...
    const renderUnavailable = (errorType) => {
        if (errorType === 'credentials_unavailable') {
            status.textContent = 'Analytics credentials unavailable or not configured.';
        } else {
        ...
        ['ga-top-pages', 'ga-traffic-sources', 'ga-geography', 'ga-devices-list'].forEach((id) => {
            ...
            if (container) {
                if (errorType === 'credentials_unavailable') {
                    renderInlineState(container, 'Analytics not configured', 'Service account credentials are missing.', true);
                } else {
                    renderInlineState(container, 'Unable to load analytics data right now.', 'The rest of the dashboard is still available.', true);
                }
            }
        });
    ...
    async function loadAnalytics() {
        ...
        try {
            const allData = await fetchAnalytics('all', range);
            const { overview, timeline, top_pages: pages, traffic_sources: sources, geography, devices } = allData;
